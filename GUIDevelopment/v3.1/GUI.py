...
class Window(tk.Tk):
    def __init__(self):
        ...
        def startcollec():
            global stop_exec
            stop_exec = False
            dist = int(cdist.get())*10 #distance between rows of pots
        ...
        Label(self, text="Distance between columns (cm)").place(x=30,y=70)
        cdist = Entry(self, validate='key', validatecommand=(vcmd, '%P'), width=5, justify='right')
        cdist.insert(END, '30')
        cdist.place(x=270,y=70)
